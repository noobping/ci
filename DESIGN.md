...
```text
.git/ci/ci -> /usr/local/bin/ci
.git/hooks/pre-commit -> ../ci/ci
.git/hooks/pre-push -> ../ci/ci
.git/hooks/post-merge -> ../ci/ci
...
```
...
```text
repo.git/ci/ci -> /usr/local/bin/ci
repo.git/hooks/pre-receive -> ../ci/ci
repo.git/hooks/post-receive -> ../ci/ci
...
```
...
```text
Remove ci-managed hook files and .git/ci/ci.
...
#!/usr/bin/env sh
# managed-by: ci
exec "$(dirname "$0")/../ci/ci" hook pre-commit "$@"
```
...
```text
link mode: symlink repo/.git/ci/ci -> current_exe
copy mode: copy current_exe -> repo/.git/ci/ci
```
